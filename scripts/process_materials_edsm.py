...
def getstars():
    starclasses = {}
    with bz2.BZ2File('../EDSM/bodies.jsonl.bz2', 'r') as f:
        for line in f:
    ...
    with bz2.BZ2File('../EDSM/bodies.jsonl.bz2', 'r') as f:
        for line in f:
            ...
            if (bodiesprocessed % 1000) == 0:
                ...
                sys.stderr.flush()
            if 'isLandable' in body and 'materials' in body and 'subType' in body and body['isLandable'] and body['materials'] and 'Iron' in body['materials']:
                totalbodies = totalbodies + 1
                lmod = calendar.timegm(time.strptime(body['updateTime'], '%Y-%m-%d %H:%M:%S'))
                btype = body['subType']
                volc = body['volcanismType'] if 'volcanismType' in body else None
                if volc is None:
                    volc = 'No volcanism'
                elif volc.startswith('Minor ') or volc.startswith('Major '):
                    volc = volc[6:]
                stemp = body['surfaceTemperature'] if 'surfaceTemperature' in body else None
                if stemp < 2:
                    stemp = None
                temp = '{}K'.format(max(v for v in tempranges if v < stemp)) if stemp else None
                
                vrares = [k for k in body['materials'].keys() if k in materials.VeryRare]
                rares = [k for k in body['materials'].keys() if k in materials.Rare]
                commons = [k for k in body['materials'].keys() if k in materials.Common]
                vcomms = [k for k in body['materials'].keys() if k in materials.VeryCommon]
                
                raregrp = ':'.join([m for m in materials.Rare if m in rares])
                commongrp = ':'.join([m for m in materials.Common if m in commons])

                if btype == 'Icy body':
                    ironpct = float(body['materials']['Iron'])
                    if ironpct >= 15.0:
                ...
                if btype not in bodymatsByType:
                    bodymatsByType[btype] = { 'count': 0, 'oddballs': [], 'materials': {} }

                if btype == 'Metal-rich body' and any(k for k in body['materials'].keys() if k in materials.VeryCommonNonMetals and body['materials'][k] != 0):
                    bodymatsByType[btype]['oddballs'] += [body]
                    ...
                    for name, share in body['materials'].iteritems():
                        ...
                        if name not in materials.VeryCommon:
                            if 'foundwith' not in bmbyname:
                                bmbyname['foundwith'] = {}
                            for name2, share2 in body['materials'].iteritems():
